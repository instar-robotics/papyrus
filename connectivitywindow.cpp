    ...
    m_outputSize(outputSize)
{
    ...
    if (gridLayout != NULL) {
        for(unsigned int i = 0; i < m_inputSize.width(); i += 1) {
            for (unsigned int j = 0; j < m_inputSize.height(); j += 1) {
//                QPushButton *btn = new QPushButton(QString::number(i+j));
    ...
    if (gridLayout != NULL) {
        for(unsigned int i = 0; i < m_outputSize.width(); i += 1) {
            for (unsigned int j = 0; j < m_outputSize.height(); j += 1) {
//                QPushButton *btn = new QPushButton(QString::number(i+j));
                QToolButton *btn = new QToolButton;
                gridLayout->addWidget(btn, i, j);
            }
